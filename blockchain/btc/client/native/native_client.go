...
var _ xclient.IClient = &NativeClient{}
var _ address.WithAddressDecoder = &NativeClient{}

// NewClient returns a new Bitcoin Client
func NewNativeClient(cfg *xc.ChainConfig) (*NativeClient, error) {
	native := cfg
...
func (client *NativeClient) FetchLegacyTxInput(ctx context.Context, from xc.Address, to xc.Address, asset xc.IAsset) (xc.TxInput, error) {
...
func (client *NativeClient) FetchLegacyTxInfo(ctx context.Context, txHash xc.TxHash) (*xc.LegacyTxInfo, error) {
	...
	if err := client.send(ctx, &resp, "gettransaction", txHash); err != nil {
		return nil, fmt.Errorf("bad \"gettransaction\": %v", err)
	}
	...
	for _, txIn := range txObject.MsgTx.TxIn {
		...
		if err != nil {
			return nil, fmt.Errorf("error retrieving input details: %v", err)
		}
		_, addresses, _, err := txscript.ExtractPkScriptAddrs(output.PubKeyScript, client.opts.Chaincfg)
		if err != nil || len(addresses) != 1 {
			return nil, fmt.Errorf("error extracting address from input: %v", err)
		}
		input := tx.Input{
			Output:  output,
	...
	for _, output := range outputs {
		...
		_, addresses, _, err := txscript.ExtractPkScriptAddrs(output.PubKeyScript, client.opts.Chaincfg)
		if err != nil || len(addresses) != 1 {
			return nil, fmt.Errorf("error extracting address from output: %v", err)
		}
...
func (client *NativeClient) UnspentOutputs(ctx context.Context, minConf, maxConf int64, addr xc.Address) ([]tx_input.Output, error) {
	...
	for i := range outputs {
		...
		outputs[i] = tx_input.Output{
			...
			Value:        xc.NewBigIntFromUint64(uint64(amount)),
...
func (client *NativeClient) FetchBalance(ctx context.Context, address xc.Address) (*xc.BigInt, error) {
...
func (client *NativeClient) EstimateGas(ctx context.Context, tx xc.Tx) (*xc.BigInt, error) {
	...
	numBlocks := 1
	fallbackGasPerByte := xc.NewBigIntFromUint64(2)
