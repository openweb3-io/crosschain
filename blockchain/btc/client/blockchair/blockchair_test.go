...
func (s *ClientTestSuite) TestFetchTxInput() {
	...
	for _, v := range testcases {
		...
		defer close()
		asset := &xc.ChainConfig{Chain: xc.BTC, URL: server.URL, Network: "testnet", AuthSecret: "1234", Provider: string(client.Blockchair), ChainMinGasPrice: 12}
		client, _ := btc_client.NewClient(asset)
...
func (s *ClientTestSuite) TestFetchTxInputUnconfirmedUtxo() {
	...
	defer close()
	asset := &xc.ChainConfig{Chain: xc.BTC, URL: server.URL, Network: "testnet", AuthSecret: "1234", Provider: string(client.Blockchair)}
