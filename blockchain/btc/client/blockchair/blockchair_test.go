...
import (
	...
	btc_client "github.com/openweb3-io/crosschain/blockchain/btc/client"
...
func TestBlockchairTestSuite(t *testing.T) {
...
func (s *ClientTestSuite) TestFetchTxInput() {
	...
	for _, v := range testcases {
		...
		defer close()
		cfg := &xc.ChainConfig{Chain: xc.BTC, URL: server.URL, Network: "testnet", AuthSecret: "1234", Provider: string(btc_client.Blockchair), ChainMinGasPrice: 12}
		client, _ := btc_client.NewClient(cfg)
...
func (s *ClientTestSuite) TestFetchTxInputUnconfirmedUtxo() {
	require := s.Require()
	...
	defer close()
	cfg := &xc.ChainConfig{Chain: xc.BTC, URL: server.URL, Network: "testnet", AuthSecret: "1234", Provider: string(btc_client.Blockchair)}
	client, _ := btc_client.NewClient(cfg)
...
func (s *ClientTestSuite) TestNewClient() {
	require := s.Require()
	asset := &xc.ChainConfig{Chain: xc.BTC, Network: "testnet", AuthSecret: "1234", Provider: string(btc_client.Blockchair)}
	client, err := btc_client.NewClient(asset)
	require.NotNil(client)
	require.NoError(err)

	asset = &xc.ChainConfig{Chain: xc.BTC, Network: "testnet", AuthSecret: "", Provider: string(btc_client.Blockchair)}
	_, err = btc_client.NewClient(asset)
