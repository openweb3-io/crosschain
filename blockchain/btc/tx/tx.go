...
func (tx *Tx) Outputs() ([]tx_input.Output, error) {
	...
	for i := range outputs {
		...
		outputs[i].Value = xc.NewBigIntFromUint64(uint64(tx.MsgTx.TxOut[i].Value))
	}
	return outputs, nil
}
...
func DetectFrom(inputs []Input) (string, xc.BigInt) {
	from := ""
	max := xc.NewBigIntFromUint64(0)
	totalIn := xc.NewBigIntFromUint64(0)
...
func (tx *Tx) DetectToAndAmount(from string, expectedTo string) (string, xc.BigInt, xc.BigInt) {
	to := expectedTo
	amount := xc.NewBigIntFromUint64(0)
	totalOut := xc.NewBigIntFromUint64(0)
