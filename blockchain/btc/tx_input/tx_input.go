...
type TxInput struct {
	...
	GasPricePerByte xc.BigInt `json:"gas_price_per_byte"`
}
...
func (input *TxInput) SetGasFeePriority(other xc.GasFeePriority) error {
	...
	input.GasPricePerByte = xc.BigInt(*gasPriceMultiplied)
	return nil
}
...
func (txInput *TxInput) GetGetPricePerByte() xc.BigInt {
...
func (txInput *TxInput) SetAmount(amount xc.BigInt) {
...
func (txInput *TxInput) SumUtxo() *xc.BigInt {
	balance := xc.NewBigIntFromUint64(0)
...
func FilterForMinUtxoSet(unspentOutputs []Output, targetAmount xc.BigInt, minUtxo int) []Output {
	filtered := []Output{}
	balance := xc.NewBigIntFromUint64(0)
...
func NewOutputs[UTXO UtxoI](unspentOutputs []UTXO, addressScript []byte) []Output {
	...
	for _, u := range unspentOutputs {
		...
		output := Output{
			...
			Value:        xc.NewBigIntFromUint64(u.GetValue()),
