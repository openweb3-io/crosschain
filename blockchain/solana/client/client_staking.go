...
func (client *Client) GetStakeAccounts(ctx context.Context, address xc_types.Address) ([]*parsedStakeAccount, error) {
	stakeAuthority, err := solana.PublicKeyFromBase58(string(address))
	if err != nil {
		return nil, err
	}
	res, err := client.client.GetProgramAccountsWithOpts(ctx, solana.StakeProgramID, &rpc.GetProgramAccountsOpts{
...
func (client *Client) FetchStakeBalance(ctx context.Context, args xclient.StakedBalanceArgs) ([]*xclient.StakedBalance, error) {
	...
	epochInfo, err := client.client.GetEpochInfo(ctx, rpc.CommitmentFinalized)
	if err != nil {
		return nil, err
	}
	...
	for _, stake := range stakeAccounts {
		...
		stakedBalance := xclient.NewStakedBalance(
			xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Stake.Delegation.Stake),
			state,
			validator,
			account,
		)
		rentReserve := xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Meta.RentExemptReserve)
...
func (client *Client) FetchStakingInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.StakeTxInput, error) {
	txInput, err := client.FetchBaseInput(ctx, args.GetFrom())
	if err != nil {
		return nil, err
	}
	// Set default fee for now
	txInput.PrioritizationFee = xc_types.NewBigIntFromUint64(100000)
	...
	voteAccounts, err := client.client.GetVoteAccounts(ctx, &rpc.GetVoteAccountsOpts{
		Commitment: rpc.CommitmentFinalized,
	})
	if err != nil {
		return nil, err
...
func (client *Client) FetchUnstakingInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.UnstakeTxInput, error) {
	stakeAccounts, err := client.GetStakeAccounts(ctx, args.GetFrom())
	if err != nil {
		return nil, err
	}
	...
	for _, stake := range stakeAccounts {
		...
		if stake.StakeAccount.Parsed.Info.Stake.Delegation.Voter == validator {
			amountStake := xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Stake.Delegation.Stake)
			amountRentReserve := xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Meta.RentExemptReserve)

			matchingStakeAccounts = append(matchingStakeAccounts, &tx_input.ExistingStake{
				ActivationEpoch:   xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Stake.Delegation.ActivationEpoch),
				DeactivationEpoch: xc_types.NewBigIntFromStr(stake.StakeAccount.Parsed.Info.Stake.Delegation.DeactivationEpoch),
...
func (client *Client) FetchWithdrawInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.WithdrawTxInput, error) {
	stakeAccounts, err := client.GetStakeAccounts(ctx, args.GetFrom())
	if err != nil {
		return nil, err
	}
