...
func (stake *StakeAccount) GetState(currentEpoch uint64) xcclient.State {
	activationEpoch := xc_types.NewBigIntFromStr(stake.Parsed.Info.Stake.Delegation.ActivationEpoch).Uint64()
	deactivationEpoch := xc_types.NewBigIntFromStr(stake.Parsed.Info.Stake.Delegation.DeactivationEpoch).Uint64()
