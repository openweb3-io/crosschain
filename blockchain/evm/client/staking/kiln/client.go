...
import (
	...
	"github.com/openweb3-io/crosschain/client/services/kiln"
...
func NewClient(rpcClient *evmclient.Client, chain *xc_types.ChainConfig, kilnCfg *services.KilnConfig) (xcclient.StakingClient, error) {
	kilnClient, err := kiln.NewClient(string(chain.Chain), kilnCfg.BaseUrl, kilnCfg.ApiToken)
	if err != nil {
		return nil, err
	}
...
func (cli *Client) FetchStakeBalance(ctx context.Context, args xcclient.StakedBalanceArgs) ([]*xcclient.StakedBalance, error) {
	...
	bal, _ := xc_types.NewAmountHumanReadableFromStr("32")
...
func (cli *Client) FetchStakingInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.StakeTxInput, error) {
	var asset xc_types.IAsset
	...
	gasLimit, err := cli.rpcClient.SimulateGasWithLimit(ctx, args.GetFrom(), exampleTf.(*tx.Tx), asset)
	if err != nil {
		return nil, err
	}
...
func (cli *Client) FetchUnstakingInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.UnstakeTxInput, error) {
	var asset xc_types.IAsset
	if as, ok := args.GetAsset(); ok {
		asset = as
	} else {
...
func (cli *Client) FetchActiveKilnValidators(ctx context.Context, from xc_types.Address) ([][]byte, error) {
...
func (cli *Client) FetchWithdrawInput(ctx context.Context, args xcbuilder.StakeArgs) (xc_types.WithdrawTxInput, error) {
