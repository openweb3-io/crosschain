...
func CalculateDepositDataRoot(amount xc_types.BigInt, publicKey []byte, cred []byte, sig []byte) ([]byte, error) {
...
func Serialize(chainCfg *xc_types.ChainConfig, publicKeys [][]byte, creds [][]byte, sigs [][]byte) ([]byte, error) {
	...
	amountH, _ := xc_types.NewAmountHumanReadableFromStr("32")
