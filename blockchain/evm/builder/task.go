...
func (txBuilder TxBuilder) NewTask(args *xcbuilder.TransferArgs, input xc.TxInput) (xc.Tx, error) {
	...
	task := asset.(*xc.TaskConfig)

	switch task.Code {
	case "WormholeTransferTx":
		return txBuilder.BuildWormholeTransferTx(args, input)
	}
	return txBuilder.BuildTaskTx(args, input)
}

func (txBuilder TxBuilder) BuildTaskPayload(args *xcbuilder.TransferArgs, input *tx_input.TxInput) (string, xc.BigInt, []byte, error) {
	asset, ok := args.GetAsset()
	if !ok {
		return "", xc.BigInt{}, nil, fmt.Errorf("not a *TaskConfig: %T", asset)
	}

	task, ok := asset.(*xc.TaskConfig)
	if !ok {
		return "", xc.BigInt{}, nil, fmt.Errorf("not a *TaskConfig: %T", asset)
	}
	...
	valueZero := xc.NewBigIntFromUint64(0)
	valueTx := args.GetAmount()
	value := valueTx
	valueConsumed := false

	// tx.to, typically contract address
	to := string(task.SrcAsset.GetContract())
	...
	if len(task.Operations) != 1 {
		return string(args.GetTo()), value, data, fmt.Errorf("expected 1 operation, got %d", len(task.Operations))
	}
	...
	case string:
		if contract == "dst_asset" {
			to = string(task.DstAsset.GetContract())
		} else if contract != "" {
			to = string(contract)
		}
	...
	for _, p := range op.Params {
		if p.Bind != "" {
			...
			case "amount":
				...
				valueConsumed = true
			case "from":
				addr := common.HexToAddress(string(args.GetFrom()))
				paddedAddr := common.LeftPadBytes(addr.Bytes(), 32)
				data = append(data, paddedAddr...)
			case "to":
				addr := common.HexToAddress(string(args.GetTo()))
				paddedAddr := common.LeftPadBytes(addr.Bytes(), 32)
				data = append(data, paddedAddr...)
			case "contract":
...
func (txBuilder TxBuilder) BuildTaskTx(args *xcbuilder.TransferArgs, input xc.TxInput) (xc.Tx, error) {
	...
	contract, value, payload, err := txBuilder.BuildTaskPayload(args, txInput)
	if err != nil {
		return nil, err
	}

	return txBuilder.gethTxBuilder.BuildTxWithPayload(native, xc.Address(contract), value, payload, input)
}

func (txBuilder TxBuilder) BuildProxyPayload(contract xc.ContractAddress, to xc.Address, amount xc.BigInt) ([]byte, error) {
...
func (txBuilder TxBuilder) BuildWormholePayload(args *xcbuilder.TransferArgs, txInput *tx_input.TxInput) (string, xc.BigInt, []byte, error) {
	asset, ok := args.GetAsset()
	if !ok {
		return "", xc.BigInt{}, nil, fmt.Errorf("not a *TaskConfig: %T", asset)
	}
	...
	priceUSD, ok := txInput.GetUsdPrice(xc.NativeAsset(task.DstAsset.GetChain().Chain), string(task.DstAsset.GetContract()))
...
func (txBuilder TxBuilder) BuildWormholeTransferTx(args *xcbuilder.TransferArgs, input xc.TxInput) (xc.Tx, error) {
	txInput := input.(*tx_input.TxInput)
	native := txBuilder.Chain
	...
	contract, value, payload, err := txBuilder.BuildWormholePayload(args, txInput)
	if err != nil {
		return nil, err
	}

	return txBuilder.gethTxBuilder.BuildTxWithPayload(txBuilder.Chain, xc.Address(contract), value, payload, txInput)
}
