...
type TxInput struct {
	...
	// DynamicFeeTx
	GasTipCap xc.BigInt `json:"gas_tip_cap,omitempty"` // maxPriorityFeePerGas
	GasFeeCap xc.BigInt `json:"gas_fee_cap,omitempty"` // maxFeePerGas
	// GasPrice xc.BigInt `json:"gas_price,omitempty"` // wei per gas
	// Task params
	...
	ChainId xc.BigInt `json:"chain_id,omitempty"`
...
func (input *TxInput) SetGasFeePriority(other xc.GasFeePriority) error {
	...
	input.GasTipCap = xc.BigInt(*multipliedTipCap)
	...
	input.GasPrice = xc.BigInt(*multipliedLegacyGasPrice)
	return nil
}
