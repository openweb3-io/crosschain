...
func (s *SpecV5R1) BuildMessage(ctx context.Context, _ bool, _ *ton.BlockIDExt, messages []*Message) (_ *cell.Builder, err error) {
	...
	/*
		var sign []byte
		if s.wallet.signer != nil {
			sign, err = s.wallet.signer.Sign(ctx, payload.EndCell().Hash())
			if err != nil {
				return nil, err
			}
		} else {
