...
func (s *CrosschainTestSuite) TestFetchTxInput() {
	...
	txInputBz, _ := json.Marshal(txInputEnvelope)
...
func (s *CrosschainTestSuite) TestFetchTxInputError() {
	...
	from := xc.Address("from")
	to := xc.Address("to")
	_, err := client.FetchLegacyTxInput(s.Ctx, from, to, nil)
...
func (s *CrosschainTestSuite) TestSubmitTx() {
	...
	err := client.BroadcastTx(s.Ctx, txData)
	require.NoError(err)
}
...
func (s *CrosschainTestSuite) TestFetchBalance() {
	...
	expectedBalance := xc.NewBigIntFromUint64(1234567)
