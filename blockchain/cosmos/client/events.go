	...
	for _, event := range events {
		...
		if event.Type == "transfer" {
			...
			for _, attr := range event.Attributes {
				...
				if attr.Key == "amount" {
					...
					transferEvent.Amount = xc.BigInt(*coin.Amount.BigInt())
		...
		if event.Type == "withdraw_rewards" {
			...
			for _, attr := range event.Attributes {
				...
				if attr.Key == "amount" {
					coin, _ := types.ParseCoinNormalized(attr.Value)
					withdrawRewardsEvent.Amount = xc.BigInt(*coin.Amount.BigInt())
		...
		if event.Type == "delegate" {
			...
			for _, attr := range event.Attributes {
				...
				if attr.Key == "amount" {
					coin, _ := types.ParseCoinNormalized(attr.Value)
					delegateEvent.Amount = xc.BigInt(*coin.Amount.BigInt())
		...
		if event.Type == "unbond" {
			...
			for _, attr := range event.Attributes {
				...
				if attr.Key == "amount" {
					coin, _ := types.ParseCoinNormalized(attr.Value)
					unbondEvent.Amount = xc.BigInt(*coin.Amount.BigInt())
		...
		if event.Type == "wasm" {
			...
			if action == "transfer" {
				...
				for _, attr := range event.Attributes {
					...
					if attr.Key == "amount" {
						transferEvent.Amount = xc.NewBigIntFromStr(attr.Value)
					}
